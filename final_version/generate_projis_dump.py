...
INPUT:  The HTML generated by the MassDOT PROJIS system for all projects in the Boston MPO area.
        This is fetched using the URL defined by the global variable projis_boston_mpo_url.
...
# URL from which HTML for all projects in the Boston MPO area is fetched:
projis_boston_mpo_url = "http://www.mhd.state.ma.us//ProjectInfo/Main.asp?ACTION=GisReportByMPO&SELECTED_MPO_CODE=2&FILTER_STATUS=&PHASE_STATUS_CODE=0&TIP_STATUS_CODE=All"
...
#     2. \n
#     3. \r
#     4. '"' - the double-quote character
#     5. "'" - the single-quote character
#
# Note that items (4) and (5) must be cleaned up so that the string doesn't
# contain any embedded quotes, which will foul up the SQL database loader.
#
...
# Get the Long Project Description, if present.
#
# This one is more than a bit subtle.
# The siblings of the <td> for "PROJ_DESC" seem to always contain a the "short"
# project description. This is found at tds[0].nextSibling.nextSibling.contents[1].string,
# where tds[0].nextSibling.nextSibling.contents[1] is a <div> tag.
# The siblings of the <td> for "PROJ_DESC" ***may*** also contain the "long" project
# description. However THIS IS NOT ALWAYS THE CASE; the long project description MAY BE MISSING!
# If present, as best we can tell, it should be tds[0].nextSibling.nextSibling.contents[3].string.
# HOWEVER, if tds[0].nextSibling.nextSibling.contents[3] exists, we can't just grab its string
# attribute blindly, because i it may NOT be a <div> tag. So, we can only feel confident extracting
# the long project description if:
#     1. The length of tds[0].nextSibling.nextSibling is >= 3 AND
#     2. tds[0].nextSibling.nextSibling.type is "div".
# Finally, if we are successful in extracting the long project descrption, we must
# "clean up" this string, as it has been found to contain non-ASCII garbage.
# Someone told me that this should be easy, but the devil's in the details, isn't it?
# And when there's no spec or other documentation, EVERYTHING is a detail.
# -- BK 12/09/09
#
# Parameters: proj_soup - the BeautifulSoup for the project page       
#             proj_dict - the dictionary being assembled for the project (IN/OUT)
#
def get_long_project_description(proj_soup, proj_dict):
        try:
                tds = proj_soup.findAll("td", id="PROJ_DESC")
                if (len(tds[0].nextSibling.nextSibling.contents) >= 3):
                        node_name = tds[0].nextSibling.nextSibling.contents[3].name
                        if (node_name == u'div'):
                                t1 = cleanup_string(tds[0].nextSibling.nextSibling.contents[3].string)
                        else:
                                t1 = ' '
                        # end_if
                else:
                ...
                return ' '
# end_def (of get_long_project_description)
...
def process_project(proj_num, proj_url, proj_dict):
        ...
        try:
                ...
                long_proj_desc = get_long_project_description(proj_soup, proj_dict)
                proj_dict['LONG_DESC'] = long_proj_desc
                ...
                # Debug
                # print "bridges_str is: " + bridges_str
                # print "length of bridges_str is: " + str(len(bridges_str))
                #
...
def write_to_csv_file(pd, csv):
        ...
        # Deal with unicode chars in the long description.       
        t1 = pd['LONG_DESC'].encode("utf-8")
        csv.write(t1 + comma)
...
# This function contains the main loop over all PROJIS projects in the Boston MPO area.
def process_all_projects(csv_file_name):
        """
        Function: process_all_projects
        This function iterates over all PROJIS projects  in the Boston MPO area, and generates
        a CSV file named csv_file_name containing the attributes extracted for each project.
        ...
        """
        global projis_boston_mpo_url, num_exceptions
        # Get HTML for all PROJIS projects in the Boston MPO region.
        projis_boston_mpo_page = urllib2.urlopen(projis_boston_mpo_url)
        # Parse the HTML read in above.
        soup = BeautifulSoup(projis_boston_mpo_page)
...
def generate_dump(output_file_name):
        """
        Function: generate_dump(output_file_name).
        ...
        process_all_projects(output_file_name)
